#!/bin/sh
# Order A, built-in camera — ours first with the lock held; QuickTime is told to pick the FaceTime HD Camera. The spike opens the camera and records (both an
...
cd "$(dirname "$0")"
DEV="${1:-FaceTime}"
OUT=out/mine-first-locked-facetime
QT="$HOME/Movies/rheocles-s1/mine-first-locked-facetime.mov"
...
osascript quicktime.applescript 20 "$QT" "FaceTime HD Camera" 2>&1 | tee "$OUT/quicktime.log"
...
echo "=== DONE mine-first-locked-facetime"
