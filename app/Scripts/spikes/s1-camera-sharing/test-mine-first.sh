#!/bin/sh
# Order A — ours first. The spike opens the camera and records (both an
...
DEV="${1:-4K X}"
OUT=out/mine-first
QT="$HOME/Movies/rheocles-s1/mine-first.mov"
rm -rf "$OUT" "$QT.qtpxcomposition"; mkdir -p "$OUT" "$(dirname "$QT")"
./in-terminal.sh "./s1 run --device '$DEV' --seconds 40 --record $OUT/mine-fileoutput.mov --writer $OUT/mine-assetwriter.mov --label mine" "$OUT/mine.log"
sleep 8
